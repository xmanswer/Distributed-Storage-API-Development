...
public class Coordinator extends Verticle {
	...
	//use prime number 31 for calculating hash code in order to achieve even distribution
	...
	/* compute given string's hash code, based on the method
	 * hashInt = charn*TOTAL^(n) + charn-1*TOTAL^(n-1) + ... 
	 * + char1*TOTAL^(n-1) + char0*TOTAL^(n-1) 
	 * where TOTAL is 31 for evenly distributing keys to hash code */
	private int getHashCode(String key) {
		int hashInt = 0;
		//go through each character in string and add up to hashInt
		for(int i = 0; i < key.length(); i++) { 
			hashInt = hashInt * prime + (int)key.charAt(i);
		}
		return hashInt;
	}
